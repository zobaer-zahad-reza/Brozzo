    ...
    try {
      ...
      guestOrders.forEach((order) => {
        order.items.forEach((item) => {
          item["orderId"] =
            order.orderId || "GUEST-" + Math.floor(Math.random() * 100000);
        ...
        <div className="flex flex-col gap-5">
          {loading ? (
            <div className="flex flex-col justify-center items-center py-24 gap-4">
              ...
              <p className="text-[10px] font-black uppercase tracking-[3px] text-zinc-600">
                Retrieving History...
              </p>
            </div>
          ) : orderData.length > 0 ? (
            orderData.map((item, index) => (
              <div
                key={index}
                className="bg-[#111113] rounded-2xl p-5 border border-zinc-900 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-zinc-700 transition-all duration-300 group"
              >
                {/* Image & Product Info */}
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-zinc-500 mt-3 max-w-xs mx-auto text-sm">
                Your order history is currently empty. Explore our collection to
